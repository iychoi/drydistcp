...
import static ua.cs.drydistcp.DistCpConstants.HDFS_RESERVED_RAW_DIRECTORY_NAME;

/**
 * The SimpleCopyListing is responsible for making the exhaustive list of all
 * files/directories under its specified list of input-paths. These are written
 * into the specified copy-listing file. Note: The SimpleCopyListing doesn't
 * handle wild-cards in the input-paths.
 */
public class SimpleCopyListing extends CopyListing {
    ...
    /**
     * Collect the list of {@literal <sourceRelativePath, sourceFileStatus>} to
     * be copied and write to the sequence file. In essence, any file or
     * directory that need to be copied or sync-ed is written as an entry to the
     * sequence file, with the possible exception of the source root: when
     * either -update (sync) or -overwrite switch is specified, and if the the
     * source root is a directory, then the source root entry is not written to
     * the sequence file, because only the contents of the source directory need
     * to be copied in this case. See
     * {@link org.apache.hadoop.tools.util.DistCpUtils#getRelativePath} for how
     * relative path is computed. See computeSourceRootPath method for how the
     * root path of the source is computed.
     *
    ...
    /**
     * Provide an option to skip copy of a path, Allows for exclusion of files
     * such as
