...
import ua.cs.drydistcp.CopyListing.*;
import ua.cs.drydistcp.mapred.CopyMapper;
import ua.cs.drydistcp.mapred.CopyOutputFormat;
...
/**
 * DistCp is the main driver-class for DistCpV2. For command-line use,
 * DistCp::main() orchestrates the parsing of command-line parameters and the
 * launch of the DistCp job. For programmatic use, a DistCp object can be
 * constructed by specifying options (in a DistCpOptions object), and
 * DistCp::execute() may be used to launch the copy-job. DistCp may
 * alternatively be sub-classed to fine-tune behaviour.
 */
...
public class DistCp extends Configured implements Tool {
    ...
    /**
     * Public Constructor. Creates DistCp object with specified
     * input-parameters. (E.g. source-paths, target-location, etc.)
     *
    ...
    /**
     * Implementation of Tool::run(). Orchestrates the copy of source file(s) to
     * target location, by: 1. Creating a list of files to be copied to target.
    ...
    /**
     * Implements the core-execution. Creates the file-list for copy, and
     * launches the Hadoop-job, to do the copy.
     *
     * @return Job handle
     * @throws Exception
     */
    ...
    /**
     * Set targetPathExists in both inputOptions and job config, for the benefit
     * of CopyCommitter
     */
     ...
     * @throws java.io.IOException - Exception if unable to locate ssl config
     * file
     */
    ...
    /**
     * Add SSL files to distributed cache. Trust store, key store and ssl config
     * xml
     *
     ...
     * implementation. Also add delegation tokens for each path to job's
     * credential store
     *
    ...
    /**
     * Get default name of the copy listing file. Use the meta folder to create
     * the copy listing file
     *
    ...
    /**
     * Create a default working folder for the job, under the job staging
     * directory
     *
     ...
     * OptionsParser), and invokes the DistCp::run() method, via the ToolRunner.
     *
