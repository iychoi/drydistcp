 ...
 */
package ua.cs.drydistcp.mapred;
...
import ua.cs.drydistcp.DistCpConstants;
import ua.cs.drydistcp.DistCpOptions.FileAttribute;
import ua.cs.drydistcp.mapred.CopyMapper.FileAction;
import ua.cs.drydistcp.util.DistCpUtils;
import ua.cs.drydistcp.util.RetriableCommand;
import ua.cs.drydistcp.util.ThrottledInputStream;

import com.google.common.annotations.VisibleForTesting;

/**
 * This class extends RetriableCommand to implement the copy of files, with
 * retries on failure.
 */
public class RetriableFileCopyCommand extends RetriableCommand {
    ...
    /**
     * Implementation of RetriableCommand::doExecute(). This is the actual
     * copy-implementation.
     *
    ...
    long drainBytes(FileStatus sourceFileStatus, long sourceOffset, int bufferSize, Mapper.Context context) throws IOException {
        Path source = sourceFileStatus.getPath();
        byte buf[] = new byte[bufferSize];
        ThrottledInputStream inStream = null;
        long totalBytesRead = 0;
     ...
     * the block size or the checksum type. Otherwise the default block size of
     * the target FS.
     */
    ...
    /**
     * Special subclass of IOException. This is used to distinguish
     * read-operation failures from other kinds of IOExceptions. The failure to
     * read from source is dealt with specially, in the CopyMapper. Such
     * failures may be skipped if the DistCpOptions indicate so. Write failures
     * are intolerable, and amount to CopyMapper failure.
     */
