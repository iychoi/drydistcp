 ...
 * Specifically, it does the following: 1. Cleanup of the meta-folder (where
 * DistCp maintains its file-list, etc.) 2. Preservation of
 * user/group/replication-factor on any directories that have been copied.
 * (Files are taken care of in their map-tasks.) 3. Atomic-move of data from the
 * temporary work-folder to the final path (if atomic-commit was opted for). 4.
 * Deletion of files from the target that are missing at source (if opted for).
