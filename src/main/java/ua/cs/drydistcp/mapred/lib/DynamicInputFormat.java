 ...
 */
package ua.cs.drydistcp.mapred.lib;
...
/**
 * DynamicInputFormat implements the "Worker pattern" for DistCp. Rather than to
 * split up the copy-list into a set of static splits, the DynamicInputFormat
 * does the following: 1. Splits the copy-list into small chunks on the DFS. 2.
 * Creates a set of empty "dynamic" splits, that each consume as many chunks as
 * it can. This arrangement ensures that a single slow mapper won't slow down
 * the entire job (since the slack will be picked up by other mappers, who
 * consume more chunks.) By varying the split-ratio, one can vary chunk sizes to
 * achieve different performance characteristics.
 */
