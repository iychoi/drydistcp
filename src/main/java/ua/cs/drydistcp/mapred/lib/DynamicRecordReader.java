...
/**
 * The DynamicRecordReader is used in conjunction with the DynamicInputFormat to
 * implement the "Worker pattern" for DistCp. The DynamicRecordReader is
 * responsible for: 1. Presenting the contents of each chunk to DistCp's mapper.
...
public class DynamicRecordReader<K, V> extends RecordReader<K, V> {
    ...
    /**
     * Implementation of RecordReader::nextValue(). Reads the contents of the
     * current chunk and returns them. When a chunk has been completely
     * exhausted, an new chunk is acquired and read, transparently.
     *
    ...
    public float getProgress()
        ...
        if (numChunksLeft < 0) {// Un-initialized. i.e. Before 1st dir-scan.
            assert numRecordsProcessedByThisMap <= numRecordsPerChunk : "numRecordsProcessedByThisMap:" + numRecordsProcessedByThisMap
