...
import ua.cs.drydistcp.CopyListing.AclsNotSupportedException;
import ua.cs.drydistcp.CopyListing.XAttrsNotSupportedException;
import ua.cs.drydistcp.CopyListingFileStatus;
import ua.cs.drydistcp.DistCpOptions;
import ua.cs.drydistcp.DistCpOptions.FileAttribute;
import ua.cs.drydistcp.mapred.UniformSizeInputFormat;
...
public class DistCpUtils {
     ...
     *
     * @param configuration The Configuration to which the value must be
     * written.
    ...
    /**
     * Returns the class that implements a copy strategy. Looks up the
     * implementation for a particular strategy from distcp-default.xml
     *
    ...
    /**
     * Gets relative path of child path with respect to a root path For ex. If
     * childPath = /tmp/abc/xyz/file and sourceRootPath = /tmp/abc Relative path
     * would be /xyz/file If childPath = /file and sourceRootPath = / Relative
     * path would be /file
     *
    ...
    /**
     * Pack file preservation attributes into a string, containing just the
     * first character of each preservation attribute
     *
    ...
    /**
     * Preserve attribute on file matching that of the file status being sent as
     * argument. Barring the block size, all the other attributes are preserved
     * by this function
     *
     ...
     * @throws IOException - Exception if any (particularly relating to
     * group/owner change or any transient error)
     */
     ...
     * @return Path of the sorted file. Is source file with _sorted appended to
     * the name
    ...
    /**
     * Determines if a file system supports ACLs by running a canary
     * getAclStatus request on the file system root. This method is used before
     * distcp job submission to fail fast if the user requested preserving ACLs,
     * but the file system cannot support ACLs.
     *
    ...
    /**
     * Determines if a file system supports XAttrs by running a getXAttrs
     * request on the file system root. This method is used before distcp job
     * submission to fail fast if the user requested preserving XAttrs, but the
     * file system cannot support XAttrs.
     *
     * @param fs FileSystem to check
     ...
     *
     * If checksums's can't be retrieved, it doesn't fail the test Only time the
     * comparison would fail is when checksums are available and they don't
     * match
     *
     ...
     * false. If checksums are retrieved, the function returns true if they
