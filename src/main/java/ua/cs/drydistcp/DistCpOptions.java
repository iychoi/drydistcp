...
/**
 * The Options class encapsulates all DistCp options. These may be set from
 * command-line (via the OptionsParser) or may be set manually.
 */
public class DistCpOptions {
     ...
     *
     * @param sourcePaths List of source-paths (including wildcards) to be
     * copied to target.
     ...
     *
     * @return true if files in target that may exist before distcp, should
     * always be overwritten. false otherwise
     */
    ...
    /**
     * Set if we want to append new data to target files. This is valid only
     ...
     * Should dry run?
     *
     * @return true if data should not be saved.
     */
    ...
    /**
     * Set if data should not be saved
     *
     * @param dryrun - boolean switch
     */
    public void setDryrun(boolean dryrun) {
        validate(DistCpOptionSwitch.DRYRUN, dryrun);
        this.dryrun = dryrun;
    }
    ...
    /**
     * Set if checksum comparison should be skipped while determining if source
    ...
    /**
     * Add file attributes that need to be preserved. This method may be called
     * multiple times to add attributes.
     *
    ...
    /**
     * Get work path for atomic commit. If null, the work path would be
     * parentOf(targetPath) + "/._WIP_" + nameOf(targetPath)
     *
    ...
    /**
     * Get output directory for writing distcp logs. Otherwise logs are
     * temporarily written to JobStagingDir/_logs and deleted upon job
     * completion
     *
    ...
    /**
     * Set the log path where distcp output logs are stored Uses
     * JobStagingDir/_logs by default
     *
    ...
    /**
     * Set the copy strategy to use. Should map to a strategy implementation in
     * distp-default.xml
     *
    ...
    public void appendToConf(Configuration conf) {
        ...
        DistCpOptionSwitch.addToConf(conf, DistCpOptionSwitch.DRYRUN,
