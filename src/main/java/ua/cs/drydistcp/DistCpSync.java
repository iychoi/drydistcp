 ...
 * 1. Both the source and target FileSystem must be DistributedFileSystem 2. Two
 * snapshots (e.g., s1 and s2) have been created on the source FS. The diff
 * between these two snapshots will be copied to the target FS. 3. The target
 * has the same snapshot s1. No changes have been made on the target since s1.
 * All the files/directories in the target are the same with source.s1
 */
class DistCpSync {
    ...
    /**
     * Compute the snapshot diff on the given file system. Return true if the
     * diff is empty, i.e., no changes have happened in the FS.
     */
