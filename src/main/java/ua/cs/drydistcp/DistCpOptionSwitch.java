...
public enum DistCpOptionSwitch {
    ...
    /**
     * Ignores any failures during copy, and continues with rest. Logs failures
     * in a file
     */
    IGNORE_FAILURES(DistCpConstants.CONF_LABEL_IGNORE_FAILURES, new Option("i", false, "Ignore failures during copy")),
    /**
     * Preserves status of file/path in the target. Default behavior with -p, is
     * to preserve replication, block size, user, group, permission, checksum
     * type and timestamps on the target file. Note that when preserving
     * checksum type, block size is also preserved.
     *
     * If any of the optional switches are present among rbugpcaxt, then only
     * the corresponding file attribute is preserved.
     */
    ...
    /**
     * Update target location by copying only files that are missing in the
     * target. This can be used to periodically sync two folders across source
     * and target. Typically used with DELETE_MISSING Incompatible with
     * ATOMIC_COMMIT
     */
    SYNC_FOLDERS(DistCpConstants.CONF_LABEL_SYNC_FOLDERS, new Option("update", false, "Update target, copying only missing" + "files or directories")),
    /**
    ...
    /**
     * Configuration file to use with hftps:// for securely copying files across
     * clusters. Typically the configuration file contains truststore/keystore
     * information such as location, password and type
     */
    ...
    /**
     * Source file listing can be provided to DistCp in a file. This allows
     * DistCp to copy random list of files from source and copy them to target
     */
    SOURCE_FILE_LISTING(DistCpConstants.CONF_LABEL_SOURCE_LISTING, new Option("f", true, "List of files that need to be copied")),
    /**
     * Copy all the source files and commit them atomically to the target This
     * is typically useful in cases where there is a process polling for
     * availability of a file/dir. This option is incompatible with SYNC_FOLDERS
     * and DELETE_MISSING
     */
    ...
    /**
     * Copy strategy is use. This could be dynamic or uniform size etc. DistCp
     * would use an appropriate input format based on this.
     */
    ...
    /**
     * Skip CRC checks between source and target, when determining what files
     * need to be copied.
     */
    ...
    /**
     * Dry run (no data storing)
     */
    DRYRUN(DistCpConstants.CONF_LABEL_DRYRUN, new Option("dryrun", false, "Transfers data without storing"));
    ...
    public String toString() {
        return super.name() + " {" + "confLabel='" + confLabel + '\'' + ", option=" + option + '}';
    }
